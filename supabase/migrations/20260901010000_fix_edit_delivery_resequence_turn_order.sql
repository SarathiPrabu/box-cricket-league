...
begin
  ...
  if (select status from public.match_innings where id = delivery_record.innings_id) <> 'live'
    or current_over_number <> (select moa.over_number from public.match_over_assignments moa where moa.id = delivery_record.over_assignment_id) then
  ...
  select exists (
    select 1 from public.match_batting_turns later
  ...
  if has_later_turn then
    ...
    if active_turn_record.id is not null then
      ...
      set status = 'ended', end_reason = 'switched', ended_at = now(), updated_at = now()
  ...
  -- The affected turn must remain active while resequencing updates delivery rows.
  ...
  if has_later_turn then
    ...
    update public.match_batting_turns
    set status = 'ended',
        end_reason = case when has_wicket then 'dismissed' when batting_turn_record.phase = 'initial' then 'six_balls' else 'switched' end,
        ended_at = coalesce(batting_turn_record.ended_at, now()), updated_at = now()
    where id = batting_turn_record.id;
