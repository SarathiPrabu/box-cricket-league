...
as $$
  with raw_stats as (
    select
      ...
      coalesce(sum(mps.runs), 0)::bigint as runs,
      coalesce(sum(mps.balls_faced), 0)::bigint as balls_faced,
      coalesce(sum(mps.fours), 0)::bigint as fours,
      coalesce(sum(mps.sixes), 0)::bigint as sixes,
      coalesce(sum(mps.balls_bowled), 0)::bigint as balls_bowled,
      coalesce(dot_stats.dot_balls_bowled, 0)::bigint as dot_balls_bowled,
      coalesce(sum(mps.runs_conceded), 0)::bigint as runs_conceded,
      coalesce(sum(mps.wickets), 0)::bigint as wickets,
      coalesce(sum(mps.catches), 0)::bigint as catches,
      coalesce(sum(mps.stumpings), 0)::bigint as stumpings,
    ...
    left join (
      select
        m_dot.season_id,
        bowler_roster.player_id,
        count(*)::bigint as dot_balls_bowled
      ...
      where md.delivery_type = 'legal'
        and md.batter_runs + md.extra_runs = 0
      group by m_dot.season_id, bowler_roster.player_id
    ) dot_stats
      on dot_stats.season_id = m.season_id
      and dot_stats.player_id = sr.player_id
    ...
    where lsps.season_id = target_season_id
      and not exists (
        select 1
        from raw_stats rs
  ...
  select
    ...
    p.slug,
    p.display_name,
    coalesce(t.name, ss.source_team_name),
    ss.matches_played,
    ss.runs,
    ss.balls_faced,
    ss.fours,
    ...
    ss.catches,
    ss.stumpings,
    ss.player_of_match_count
  from season_stats ss
  join public.players p on p.id = ss.player_id
  left join public.season_rosters sr
    on sr.season_id = ss.season_id
    and sr.player_id = ss.player_id
  left join public.season_teams st on st.id = sr.season_team_id
  left join public.teams t on t.id = st.team_id
  where ss.season_id = target_season_id
